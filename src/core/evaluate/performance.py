...
from core.evaluate.metrics import *


def evaluate(user_emb, item_emb, train_U2I, test_U2I, args):
    scores = np.matmul(user_emb, item_emb.T)
    topks = args.topks
    test_user_set = [k for k, v in test_U2I.items() if len(v) > 0]
    ...
    for i, one_perf in enumerate(all_perf):
        perf_info[i] = one_perf['perf']
    perf_info = np.mean(perf_info, axis=0)

    return perf_info, all_perf
...
def test_one_perf(X):
    ...
    result = {
        'uid': u_id,
        'perf': get_perf(score_indices, uid_test_pos_items, topks)
    }
    return result
...
def get_perf(rank, uid_test_pos_items, topks):
    topk_eval = np.zeros(3 * len(topks), dtype=np.float32)
    for i, topk in enumerate(topks):
        topk_eval[i * 3 + 0] = ndcg_k(rank[:topk], uid_test_pos_items)
        topk_eval[i * 3 + 1] = hr_k(rank[:topk], uid_test_pos_items)
        topk_eval[i * 3 + 2] = recall_k(rank[:topk], uid_test_pos_items)
    return topk_eval
